...
from tools.normdev.harness import (
    DEFAULT_AX,
    NormStore,
    capture_env,
    gradient,
...
def run_smoke(base: Path, *, frames: Path) -> Smoke:
    ...
    a = write_fake_frame(frames / "a", image=gradient(), ax=DEFAULT_AX, app="TextEdit")
    r = store.run("record", "--once", "--interval", "1", extra_env=capture_env(a))
    ...
    print("record a changed frame -> stored")
    b = write_fake_frame(frames / "b", image=gradient(vertical=True), ax=DEFAULT_AX, app="Safari")
    store.run("record", "--once", "--interval", "1", extra_env=capture_env(b))
    s.check(len(_captures(store)) == 2, "second capture stored")

    print("record while idle -> skipped")
    r = store.run(
        "record", "--once", "--idle-threshold", "1", extra_env=capture_env(a, idle="600")
    )
